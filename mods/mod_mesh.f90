module mod_mesh
    ...
    type, extends(Element) :: Face
        ...
        integer(int32), dimension(2) ::                 face_adjacent_faces ! cell local index
    end type Face
    
    type, extends(Element) :: Cell
        integer(int32), dimension(3) ::                 cell_nodes
        integer(int32), dimension(3) ::                 cell_faces
        ! integer(int32), dimension(3) ::                 cell_cells
        real(real64) ::                                 cell_volume
        real(real64), dimension(3) ::                   cell_face_signs
        ! integer(int32), dimension(3) ::                 cell_adjacent_faces ! cell local index
...
contains
    ...
    subroutine set_boundary_faces(self, c1, l1, l2, file_unit, k)
        ...
        do i = 1, k
            ...
            do j = 1, self % n_face_types(i)
                ...
                if (i == 1) then ! freestream
                    l1 = [l1, n1 + 1]
                    l2 = [l2, n2 + 1]
                else ! slipwall
                    l1 = [l1, n2 + 1] ! IMPORTANT
                    l2 = [l2, n1 + 1] ! IMPORTANT
                end if
            end do
        end do
    ...
    ! subroutine set_cell_cells(self)
    !     type(Mesh), intent(in out) :: self
    ...
    !                 self % cells(i) % cell_cells(j) = &
    !                 self % faces(fj) % face_cells(2)
    !             else
    !                 self % cells(i) % cell_cells(j) = &
    !                 self % faces(fj) % face_cells(1)
    !             end if
    !         end do
    !     end do
    ! end subroutine set_cell_cells
    ...
    subroutine set_face_components(self)
        ...
        do i = 1, self % n_faces
            ...
            do j = 1, 2
                ...
                self % faces(i) % face_adjacent_faces(j) = &
    ...
    subroutine set_cell_components(self)
        ...
        do i = 1, self % n_cells
            ...
            do j = 1, 3
                fj = self % cells(i) % cell_faces(j)
                ! cj = self % cells(i) % cell_cells(j)
                if (i == self % faces(fj) % face_cells(1)) then
                    self % cells(i) % cell_face_signs(j) = 1.0
                else
                    self % cells(i) % cell_face_signs(j) = -1.0
                end if
                ! self % cells(i) % cell_adjacent_faces(j) = &
                ! findloc(self % cells(cj) % cell_faces(:), fj, 1)
            end do
        end do
