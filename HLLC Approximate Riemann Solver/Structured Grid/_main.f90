...
contains
    ...
    pure function get_yflux(i, j, Qnb, Qsb) result(GBT)
        ...
        real :: rT, uT, vT, pT, ET
        real :: aB, aT, sB, sT
        real :: r_bar, a_bar, p_bar, p_star, s_star, wB, wT
        real, dimension(4) :: QB_star, QT_star, GB_star, GT_star
        ...
        if (p_star <= pB) then
            ...
            wB = sqrt(1 + (g + 1)/(2*g)*(p_star/pB - 1))
        end if

        if (p_star <= pT) then
            wT = 1.
        else
            wT = sqrt(1 + (g + 1)/(2*g)*(p_star/pT - 1))
        end if
        
        sB = vB - aB*wB
        sT = vT + aT*wT
        s_star = (pT - pB + rB*vB*(sB - vB) - rT*vT*(sT - vT))/(rB*(sB - vB) - rT*(sT - vT))
        ...
        QB_star = rB*(sB - vB)/(sB - s_star)*(/1., uB, s_star, EB/rB + (s_star - vB)*(s_star + pB/rB/(sB - vB))/)
        QT_star = rT*(sT - vT)/(sT - s_star)*(/1., uT, s_star, ET/rT + (s_star - vT)*(s_star + pT/rT/(sT - vT))/)
        GB_star = GB + sB*(QB_star - QB)
        GT_star = GT + sT*(QT_star - QT)

        if (sB > 0.) then
            GBT = GB
        else if (sB <= 0. .and. s_star >= 0.) then
            GBT = GB_star
        else if (s_star <= 0. .and. sT >= 0.) then
            GBT = GT_star
        else if (sT < 0.) then
            GBT = GT
        end if
