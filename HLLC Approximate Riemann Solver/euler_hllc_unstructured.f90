program euler
    ...
    real, parameter :: r = 1.
    real, parameter :: uf = 1.5 ! final Mach
    real, parameter :: ui = 1. ! initial Mach
    real, parameter :: uk = .5 ! ramp factor (r-curve)
    ! real, parameter :: ak = .5 ! ramp factor (s-curve)
    ! real, parameter :: tk = 15. ! translation factor (s-curve)
    real, parameter :: v = 0.
...
! mesh complete
    ...
    Qi = [r, r*ui, r*v, .5*r*(ui*ui + v*v) + p/(g - 1)] ! inlet/initial conditions
    Q(:, 1) = Qi(1)
    Q(:, 2) = Qi(2)
    Q(:, 3) = Qi(3)
    Q(:, 4) = Qi(4)
...
! loop start
    ...
    do n = 1, int(t/dt) + 1
        ...
        u = ui + (uf - ui)*(1 - uk**(n*dt)) ! r-curve
        ! u = ui + (uf - ui)/(1 + exp(1.)**(-ak*(n*dt - tk))) ! s-curve
        ...
        print *, int(t/dt) + 1, n, u, maxval(Q(:, 1)), maxval(Co)
