...
for n in range(1, N):
    if n % nint == 0:
        ...
        read = open(os.path.join(folder, file + '{:010d}.txt'.format(n)), 'r').readlines()
        ...
        plt.colorbar()
        plt.axis('equal')
        plt.savefig(os.path.join(folder, file + '{:010d}.png'.format(n)))
        plt.clf()

image = []
for n in range(1, N):
    if n % nint == 0:
        image.append(os.path.join(folder, file + '{:010d}.png'.format(n)))
